...
class Lead(Base):
    __tablename__ = "leads"
    __table_args__ = (
        UniqueConstraint("fb_lead_id", name="uq_leads_fb_lead_id"),
        Index("ix_leads_funnel_external", "funnel_form_id", "external_lead_id", unique=True),
    )
    ...
    # external_lead_id = Google Sheet row ID / fb lead id
    external_lead_id: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
    ...
    lead_created_time: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    raw_data_json: Mapped[str | None] = mapped_column(Text, nullable=True)
    # delivered_admin=False means admin was NOT notified (by design for routine leads)
    # delivered_telegram used only as "any client delivery succeeded"
    ...
    delivered_recipients_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    ...
    def __repr__(self) -> str:
        return f"<Lead id={self.id} external={self.external_lead_id!r}>"
